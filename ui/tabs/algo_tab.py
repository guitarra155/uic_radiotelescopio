...
def build_algo_tab(page: ft.Page, algo_key: str,
    ...
    # Estado y métricas
    status_txt   = ft.Text("Esperando resultados...", color=C.TEXT_MUTED,
                           size=11, italic=True)
    ...
    side = panel(
        ...
        content=ft.Column([
            ...
            ft.Text(info.get("full_name", title),
                    color=C.TEXT_MAIN, size=11, weight=ft.FontWeight.W_600),
            ft.Container(height=4),
            ft.Text(info.get("desc", ""), color=C.TEXT_MUTED, size=10),

            ft.Divider(color=C.BORDER_COL, height=10),
            ft.Text("⚙️  Parámetros", color=C.ACCENT_CYAN, size=12,
                    weight=ft.FontWeight.BOLD),
            ft.Text(info.get("params", ""), color=C.TEXT_MUTED, size=10),

            ft.Divider(color=C.BORDER_COL, height=10),
            ft.Text("📊  Estado", color=C.ACCENT_CYAN, size=12,
                    weight=ft.FontWeight.BOLD),
            status_txt,
            ft.Container(height=4),
            peaks_count,
