...
offset_calibracion = -60; % Calibración basada en 'RadioTelescopio'
...
dir_base_dat = fullfile('Resultados_Datos', ['AR_' name_no_ext]);
...
dir_base_img = fullfile('Resultados_Imagenes', ['AR_' name_no_ext], [timestamp_inicio folder_suffix]);
...
if ~run_batch_mode
    fig3D=figure('Name', ' AR 3D', 'Color','w'); ax3D=axes; hold(ax3D,'on');
    title(sprintf(' AR | %s', name_no_ext), 'Interpreter', 'none');
...
for k=1:numBlocks
    ...
    for i=1:num_seg
        try
            [P, ~] = pyulear(segments(:,i), orden_ar, nfft, fs);
            P_blk(:,i) = fftshift(P);
        catch
...
if ~run_batch_mode
    ...
    % saveas(fig2D, fullfile(dir_base_img, ['AR_2D_ConMarcadores_' name_no_ext '.fig']));
    saveas(fig3D, fullfile(dir_base_img, ['AR_3D_ConMarcadores_' name_no_ext '.png']));
    saveas(fig3D, fullfile(dir_base_img, ['AR_3D_ConMarcadores_' name_no_ext '.fig']));
end
...
if ~run_batch_mode
    ...
    title('Estimación Espectral (AR)');
    saveas(figProfile, fullfile(dir_base_img, ['AR_Perfil_' name_no_ext '.png']));
    saveas(figProfile, fullfile(dir_base_img, ['AR_Perfil_' name_no_ext '.fig']));
end

disp('Proceso AR completado.');
...
if run_batch_mode
    ...
    results_batch.AR.f = f_abs;
    results_batch.AR.P = spec_profile;
end
